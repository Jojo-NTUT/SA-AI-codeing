...
| Read-only Entity | `src/main/java/{rootPackage}/{aggregate}/entity/ReadOnly{ChildEntity}.java` (when exposing mutable child entities) |
...
- `read-only-entity.md` governs any aggregate accessor that returns mutable child entities.
...
### Rule 13: Read-only Entity Exposure

<!-- @authority: read_only_entity_exposure | source: patterns/domain/read-only-entity.md -->

Aggregate roots MUST NOT return mutable child entity references directly to external callers.

When an accessor returns a mutable child entity, return a `ReadOnly{Entity}` instance. When an accessor returns a collection of mutable child entities, map every item to its read-only version and return an unmodifiable collection.
...
Immutable value objects, IDs, enums, primitives, strings, and immutable child entity records may be returned directly.
...
| Entity exposure | Mutable child entities are returned as `ReadOnly{Entity}` |
...
### Step 9.7: Generate Read-only Entity Accessors

If an accessor returns a mutable child entity, follow `read-only-entity.md`.

```java
public ${ChildEntity} get${ChildEntity}(${ChildEntity}Id id) {
    ${ChildEntity} entity = ${collection}.get(id);
    return entity == null ? null : new ReadOnly${ChildEntity}(entity);
}

public List<${ChildEntity}> get${ChildEntityPlural}() {
    return ${collection}.values().stream()
            .map(ReadOnly${ChildEntity}::new)
            .map(${ChildEntity}.class::cast)
            .toList();
}
```

Do not generate direct mutable entity returns from aggregate accessors.
